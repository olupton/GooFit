...
public:
  ...
  AmplitudeInfo(Variable *ar = NULL, Variable *ai = NULL, Variable *ard = NULL, Variable *ari = NULL, bool mode_ = true)
    : real(ar), imag(ai), real_del(ard), imag_del(ari), mode(mode_)
  {
  }
};
...
public:
  ...
  ResonancePdf (string name,
      ...
      bool useUnNormalisedDampingFactors = false);
  
  enum CouplingTreatment { MULTIPLY_BY_NOMINAL_MASS = 0, SQUARE };
  enum WhichMeson { F = 0, A };
  enum MesonCharge { CHARGED, NEUTRAL };
  enum FormFactorType { EXPPOLY = 0 , POLY, RECURSIVEPOLY, SENSIBLEPOLY, NORMPOLY, NORMEXPPOLY, CENTRENORMEXPPOLY,
    CENTRENORMEXPPOLYRECURSIVE, CENTRENORMEXPPOLYDEEPRECURSIVE, CENTRENORMEXPPOLYDEEPRECURSIVEALT };
  ...
  ResonancePdf(string name, const AmplitudeInfo &amp_);
  ...
  const AmplitudeInfo& getAmp() { return amp; }
private:
